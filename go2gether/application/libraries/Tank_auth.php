...
class Tank_auth
{
	...
	//validate user email while registration
	function validateEmail($email){
		
		echo $this->ci->users->is_email_available($email);
	}
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if ($user = $this->ci->users->$get_user_func($login)){	// login ok
	 ...
	 * @return	array
	 */
	function create_user($fname,$lname, $email, $password,$dob,$gender,$working, $email_activation)
	{
			...
			$data2= array(
				...
				'personal_lname'	=> $lname,
				'personal_email'	=> $email,
				'personal_gender'	=> $gender,
				'personal_company'	=> $working,
				'personal_dob'	=> $dob,
			);
			...
			if ($email_activation) {
			    ...
			    $res = $this->ci->users->create_user($data,$data2, !$email_activation);
				$this->ci->session->set_userdata(array(
								'user_id'	=> $res['user_id'],
								'username'	=> $fname,
								'status'	=> '1',
